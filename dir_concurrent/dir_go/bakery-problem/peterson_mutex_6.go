...
const (
	NrOfProcesses = 2 // Peterson's algorithm is for 2 processes only
	MinSteps      = 50
	MaxSteps      = 100
...
type PetersonMutex struct {
	flag [2]int32 // Array of atomic integers (0 = false, 1 = true)
	turn int32    // Atomic integer for turn (0 or 1)
}

func NewPetersonMutex() *PetersonMutex {
	return &PetersonMutex{
		flag: [2]int32{0, 0}, // Both processes initially don't want to enter
		turn: 0,              // Process 0 starts with turn
	}
}

func (pm *PetersonMutex) setFlag(processID int, value bool) {
	if value {
		atomic.StoreInt32(&pm.flag[processID], 1)
	} else {
		atomic.StoreInt32(&pm.flag[processID], 0)
	}
}

func (pm *PetersonMutex) getFlag(processID int) bool {
	return atomic.LoadInt32(&pm.flag[processID]) == 1
}

func (pm *PetersonMutex) getTurn() int {
	return int(atomic.LoadInt32(&pm.turn))
}

func (pm *PetersonMutex) setTurn(processID int) {
	atomic.StoreInt32(&pm.turn, int32(processID))
}

func (pm *PetersonMutex) Lock(processID int) {
...
func NewProcess(id int, symbol rune, mutex *PetersonMutex, startTime time.Time) *Process {
	return &Process{
		id:        id,
		symbol:    symbol,
		mutex:     mutex,
...
func (p *Process) run(wg *sync.WaitGroup, traceChan chan<- []TraceEntry) {
	...
	for step := 0; step < nrOfSteps/4; step++ {
		...
		// Peterson's algorithm exit protocol
		p.mutex.Unlock(p.id)
